...
#    - x resolution
#    - y resolution
background_file = '/home/ignace/Documents/PhD/TELEMAC/Playing_Around/First_Steps/Test_Cases/input/v1/Churute_Subregion.bg' # directory to store the field file
outside_mesh_size = 200 # mesh size outside the given mesh size field
multiplier = 0.33
minimum_resolution = 5
interpolate_buffer_thickness = 200
...
if generate_geo_file:
    ...
    if inner_flag:
        ...
        if len(inter)>0:
            ...
            # create a list to store the line ids per ring
            Inner_line_ids = []
            ...
            for I in Inner_Points:
                ...
                Inner_line_ids.append([n_Spline_inter])
                n_Spline_inter += 1
                # add all points to this spline
                for n in range(1,len(I)):
                    i = I[n]
                    f.write('%d};\nLine(%d) = {%d,' % (i, n_Spline_inter, i))
                    Inner_line_ids[-1].append(n_Spline_inter)
                    n_Spline_inter += 1
