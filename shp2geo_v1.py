...
outputfile = '/home/ignace/Desktop/test.geo'
#outputfile = '/home/ignace/Documents/PhD/TELEMAC/Playing_Around/GMSH/test8.geo'

# --- Mesh Properties --- #
heterogenuous = False
homo_mesh_size = 50 # if heterogenuous is False and thus a homogeneous mesh is requested
XY_field = '/home/ignace/Documents/PhD/TELEMAC/Playing_Around/GMSH/Churute_MeshSize_NumpyArray.pck' # if heterogenuous is True
...
#    - x resolution
#    - y resolution
background_file = '/home/ignace/Documents/PhD/TELEMAC/Playing_Around/First_Steps/Churute_toy_heter/input/Churute_toy_v13_Heter.bg' # directory to store the field file
OutsideMeshSize = 250 # mesh size outside the given mesh size field
...
if generate_geo_file:
    ...
    # only proceed if inner rings should be considered (indicated by a flag)
    if inner_flag:
        # check if there are inner rings
        if len(inter) > 0:
            # create header
    ...
    # if the cell size is set as heterogenuous
        ...
        np2BackgroundField(xy[0], background_file ,minres = 50, zeropoint= xy[1], xres = xy[2], yres = xy[3])
        ...
        f.write('Field[1].SetOutsideValue = %f;\n' % (OutsideMeshSize))
    ...
    min = t//60
    sec = t%60
    print('Ready! .geo-file is written in %s. It took %.2f minutes and %.2f seconds.' % (outputfile, min, sec))
